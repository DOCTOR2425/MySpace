...
namespace InstrumentStore.Domain.Services
{
	public class FillDataBaseService : IFillDataBaseService
	{
			...
			string countryName)
		{
			...
			{
				result.Brand = await _brandService.GetById(
					await _brandService.Create(brandName));
			}
			...
			{
				result.Country = await _countryService.GetById(
					await _countryService.Create(countryName));
			}
