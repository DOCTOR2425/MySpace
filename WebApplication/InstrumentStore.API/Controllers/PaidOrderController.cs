...
namespace InstrumentStore.API.Controllers
{
	...
	public class PaidOrderController : ControllerBase
	{
		...
		[Authorize(Roles = "admin")]
		[HttpGet("get-user-paid-orders-for-admin/{userId:guid}")]
		public async Task<IActionResult> GetUserPaidOrdersForAdmin(
			[FromRoute] Guid userId)
		{
			List<PaidOrder> orders = await _paidOrderService.GetAllByUserId(userId);
			List<AdminPaidOrderResponse> response = new List<AdminPaidOrderResponse>();
